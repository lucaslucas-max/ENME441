...
GPIO.setup(switch, GPIO.IN, pull_up_down=GPIO.PUD_DOWN)  # fixed variable name
...
def switch_callback(channel):
    ...
    print(f"Switch pressed! Direction = {'normal' if direction == 1 else 'reversed'}")

# --- Add threaded event detection ---
GPIO.add_event_detect(switch, GPIO.RISING, callback=switch_callback, bouncetime=300)

try:
	while True:
		t = time.time() - start
		# Adjust phase direction by flipping sign of theta
		B = (math.sin(2*math.pi*f*t))**2
		Btwo = (math.sin(2*math.pi*f*t - direction*theta))**2
		Bthree = (math.sin(2*math.pi*f*t - direction*theta*2))**2
		Bfour = (math.sin(2*math.pi*f*t - direction*theta*3))**2
		Bfive = (math.sin(2*math.pi*f*t - direction*theta*4))**2
		Bsix = (math.sin(2*math.pi*f*t - direction*theta*5))**2
		Bseven = (math.sin(2*math.pi*f*t - direction*theta*6))**2
		Beight = (math.sin(2*math.pi*f*t - direction*theta*7))**2
		Bnine = (math.sin(2*math.pi*f*t - direction*theta*8))**2
		Bten = (math.sin(2*math.pi*f*t - direction*theta*9))**2
